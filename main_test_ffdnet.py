...
def main():
    ...
    model_name = 'ffdnet_gray_clip'           # 'ffdnet_gray' | 'ffdnet_color' | 'ffdnet_color_clip' | 'ffdnet_gray_clip'
    testset_name = 'FINAL_TEST'               # test set,  'bsd68' | 'cbsd68' | 'set12'
    ...
    # gt_name = 'genoray_GT'
    # H_path = os.path.join(testsets, gt_name)                      # H_path, for High-quality images
    ...
    for idx in tqdm(range(start_idx, len(L_paths)), desc="Processing images"):
        img = L_paths[idx]
        start_time = time.time()
        print(len(L_paths), idx, img)
        # ------------------------------------
        # (1) img_L
        # ------------------------------------
        ...
        tqdm.write(f"[{idx+1}/{len(L_paths)}] {img_name+ext} processed in {elapsed:.2f} sec")
    
    total_elapsed = time.time() - total_start_time
    print(f"\n✅ All {len(L_paths)} images processed in {total_elapsed:.2f} seconds.")
