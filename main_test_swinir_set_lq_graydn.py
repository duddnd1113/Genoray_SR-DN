...
def main():
    ...
    parser.add_argument('--model_path', type=str,
                        default='model_zoo/004_grayDN_DFWB_s128w8_SwinIR-M_noise15.pth')
    parser.add_argument('--folder_lq', type=str, default = 'RESIZED_TEST', help='input low-quality test image folder')
    parser.add_argument('--folder_gt', type=str, default = 'RESIZED_TEST', help='input ground-truth test image folder')
    ...
    for idx, path in enumerate(tqdm(image_paths, desc="Processing Images", total=total_images, ncols=100, leave=True)):
        ...
        with torch.no_grad():
            ...
            elapsed_times.append((imgname, elapsed)) 
        ...
        # 노이즈 랜덤하게 추가한 LQ 이미지도 저장
        img_lq_np = img_lq.squeeze().float().cpu().clamp_(0, 1).numpy()  # shape: (H, W)
        img_lq_np = (img_lq_np * 255.0).round().astype(np.uint8)
        cv2.imwrite(f'{save_subdir}/{imgname}_noisy_input.png', img_lq_np)
    ...
    #     for name, t in elapsed_times:
    #         print(f"{name:20s}: {t:.4f} seconds")
    ...
    # if img_gt is not None:
    #     # 기본 평균값 계산
    #     ave_psnr = sum(test_results['psnr']) / len(test_results['psnr']) if test_results['psnr'] else None
    #     ave_ssim = sum(test_results['ssim']) / len(test_results['ssim']) if test_results['ssim'] else None


    #     print('\n{} \n-- Average PSNR/SSIM(RGB): {:.2f} dB; {:.4f}'.format(save_dir, ave_psnr, ave_ssim))

    #     if img_gt.ndim == 3 and test_results['psnr_y'] and test_results['ssim_y']:
    #         ave_psnr_y = sum(test_results['psnr_y']) / len(test_results['psnr_y'])
    #         ave_ssim_y = sum(test_results['ssim_y']) / len(test_results['ssim_y'])
    #         print('-- Average PSNR_Y/SSIM_Y: {:.2f} dB; {:.4f}'.format(ave_psnr_y, ave_ssim_y))

    #     if args.task in ['jpeg_car', 'color_jpeg_car'] and test_results['psnrb']:
    #         ave_psnrb = sum(test_results['psnrb']) / len(test_results['psnrb'])
    #         print('-- Average PSNRB: {:.2f} dB'.format(ave_psnrb))

    #         if args.task == 'color_jpeg_car' and test_results['psnrb_y']:
    ...
    #     csv_filename = f"{args.task}_{now_str}.csv"
...
def get_image_pair(args, path):
    ...
    print(f"Processing image: {imgname}{imgext}")
